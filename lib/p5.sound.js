/** [p5.sound]  Version: 1.0.0 - 2021-04-12 */ 
 /**
...
/* harmony default export */ var master = (p5sound);
...
function soundFormats() {
  ...
  master.extensions = []; // add extensions
...
function () {
  function SoundFile(paths, onload, onerror, whileLoading) {
    ...
    this.panner = new panner_0(this.output, master.input, 2); // it is possible to instantiate a soundfile with no path
  ...
  soundfile_createClass(SoundFile, [{
    ...
    value: function dispose() {
      var now = master.audiocontext.currentTime; // remove reference to soundfile
...
function () {
  function Amplitude(smoothing) {
    ...
    master.meter.connect(this._workletNode); // add this p5.SoundFile to the soundArray
...
function () {
  function Oscillator(freq, type) {
    ...
    this.oscillator.frequency.setValueAtTime(this.f, master.audiocontext.currentTime); // connections
    ...
    this.panPosition = 0.0;
    this.connection = master.input; // connect to p5sound by default
    ...
    master.soundArray.push(this); // these methods are now the same thing
  ...
  oscillator_createClass(Oscillator, [{
    ...
    value: function phase(p) {
      ...
      if (!this.dNode) {
        // create a delay node
        this.dNode = master.audiocontext.createDelay(); // put the delay node in between output and panner
...
function (_Oscillator) {
  ...
  noise_createClass(Noise, [{
    ...
    key: "dispose",
    value: function dispose() {
      var now = master.audiocontext.currentTime; // remove reference from soundArray
...
function (_Oscillator) {
  ...
  function Pulse(freq, w) {
    ...
    _this.dNode = master.audiocontext.createDelay(); // dc offset
...
function () {
  ...
  polysynth_createClass(PolySynth, [{
    ...
    value: function noteAttack(_note, _velocity) {
      ...
      var acTime = master.audiocontext.currentTime + secondsFromNow; //Convert note to frequency if necessary. This is because entries into this.notes
...
p5.PolySynth = polysynth; // Following are the deprecated classes
